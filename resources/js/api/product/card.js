...
import {
    CompraRecorrenteCart,
    CompraRecorrenteStorage,
...
function insertItemInList(productId, element) {
    let keep = true,
        variationSelected = '',
        $parent =
            $(element).closest('.ui.card.produto').length == 0
                ? $(element).closest('.item.produtoList')
                : $(element).closest('.ui.card.produto')

    $parent
        .find('.sku-options [id=referencefromproduct_' + productId + ']')
        .each(function() {
            ...
            if (idVariation === '') {
                ...
                variationSelected +=
                    variationSelected !== '' ? ',' + idVariation : idVariation
                keep = true
            }
        })
...
$(document).on('click', '#btn_compre_conjunto_selecionado', function(e) {
    ...
    if ($product_selected.length > 0) {
        ...
        _alert(
            '',
            'Selecione ao menos um produto, antes de adicionar ao carrinho.',
            'error'
        )
    }
})

$(document).on('click', '#btn_compre_oneclick_conjunto_selecionado', function(
    e
) {
    let $product_selected = $('.conjunct_product:checked')
    if ($product_selected.length > 0) {
...
$(document).on('click', '.button.avise-card', function() {
    ...
    $.ajax({
        ...
        data: {
            produtoID:
                typeof $('#produto-id').val() !== 'undefined'
                    ? $('#produto-id').val()
                    : $(this).data('idproduct'),
            sku:
                typeof $('#produto-sku').val() !== 'undefined'
                    ? $('#produto-sku').val()
                    : $(this).data('produto-sku'),
            titulo:
                $('#produto-nome').text() !== ''
                    ? $('#produto-nome').text()
                    : $(this).data('name'),
            imagem: $('#mainImageCard_' + $(this).data('idproduct')).attr(
                'src'
            ),
            codigo:
                typeof $('#produto-codigo').val() !== 'undefined'
                    ? $('#produto-codigo').val()
                    : $(this).data('produto-codigo'),
        },
...
$(document).on('change', '.dropdownreference', function() {
    ...
    $('.sku-options [id=referencefromproduct_' + idProduct + ']')
        .not(this)
        .each(function() {
            $(this)
                .find('.item')
...
$(document).ready(function() {
    ...
    if ($('.conjunct_product:checked').length > 0) {
            ...
            $(element)
                ...
                .each(function() {
                    ...
                    for (let variation in variations) {
                        $that.dropdown(
                            'set selected',
                            variations[variation].IdVariation
                        )
                    }
                })
        ...
        $('.buy-conjunct').removeClass(
            `ui animated ${color_comprar_button_class} button fluid`
        )
        $('.buy-conjunct').addClass(
            `ui labeled icon button fluid ${color_aviseme_button_class} disabled`
        )
...
export function insertItemInCart(
    productId,
    element,
    showSideBar,
    signature = false
    ...
    if (variationSelected.status) {
        callAjaxInsertItemInCart(
            productId,
            variationSelected.ids,
            1,
            element,
            showSideBar,
            signature
        ...
        */
        $.ajax({
            ...
            success: function(response) {
                ...
                var ponteiroPricesCurrent = $parent.find(
                    '#basePrice_' + productId + '> i'
                ).length

                if (sku.Price > 0 && sku.PricePromotion > 0) {
                    if (ponteiroPricesCurrent > 0) {
                        $parent
                            .find('#basePrice_' + productId + '> i')
                            .text(moneyPtBR(sku.Price))
                    } else {
                ...
                if (sku.InstallmentMax) {
                    $parent
                        .find('.installmentMaxNumber')
                        .text(sku.InstallmentMax.MaxNumber)
                    $parent
                        .find('.installmentMaxValue')
                        .text(moneyPtBR(sku.InstallmentMax.Value))
                }

                if (sku.Stock <= 0 || isExhausted(productId)) {
                    $parent
                        .find('#btn-comprar-card-' + productId)
                        .removeClass(
                            `${color_comprar_button_class} btn-comprar-card`
                        )
                        .addClass(
                            `${color_aviseme_button_class} avise-card avise-me-modal`
                        )
                        .html('<i class="icon announcement"></i> Avise-me')

                    $parent
                        .find('#btn-assinatura-card-' + productId)
                        .addClass(`hideme`)

                    $parent
                        .find('#add-event-list-' + productId)
                        .removeClass(
                            `${color_comprar_button_class} btn-comprar-card`
                        )
                        .addClass(
                    ...
                    $parent
                        .find('#produto-esgotado_' + productId)
                        .removeClass('hideme')
                    ...
                    $parent
                        ...
                        .addClass(
                            `ui labeled icon button ${color_comprar_button_class} fluid add-event-list`
                        )
                        .html(
                            '<i class="icon add to cart"></i> Adicionar à Lista'
                        )
...
export function callAjaxInsertItemInCart(
    idProduct,
    variations,
    quantity,
    element,
        ...
        success: function(response) {
            if (response.success) {
                ...
                if ($(CompraRecorrenteCart.modalConfig.id).length > 0)
                    $(CompraRecorrenteCart.modalConfig.id).attr(
                        'data-active',
                        signature
                    )

                // Limpa a Storage caso um produto sem recorrencia seja adicionado
                if (!signature) CompraRecorrenteStorage.cleanStorage()
            } else {
                $(document)
                    .find('.loading')
                    .removeClass('loading')
...
function updateStockConjunct(element) {
    ...
    if (stock <= 0 || isExhausted(productId)) {
        ...
        $button.next('input.conjunct_product').prop('checked', false)
        ...
        $button.next('input.conjunct_product').prop('checked', true)
        return true
    }
}
...
export function getVariationIds(parentElement, productId) {
    let variationSelected = '',
        _return = {},
        $skuOptions = $(parentElement).find(
            '.sku-options [id=referencefromproduct_' + productId + ']'
        )
